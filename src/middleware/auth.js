...
const auth = async (req, res, next) => {
  try {
    ...
    res.status(401).send({ error: "Please authenticcate" });
  }
};
...
// If the user exists, the middleware calls next(
// ) to pass control to the next function in the request pipeline. Without next() the remaing code will not run.
